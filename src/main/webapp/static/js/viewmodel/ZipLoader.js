...
(function(global, factory) {
	...
	*/
	...
	var parseZip = function parseZip(buffer,compressSize) {
		...
		while(true) {
			...
			if(signature === LOCAL_FILE_HEADER) {
				...
				var file = parseLocalFile(reader,compressSize);
				files[file.name] = {
					buffer: file.buffer
				};
				continue;
	...
	var parseLocalFile = function parseLocalFile(reader,compressSize) {
		...
		switch(compression) {
			...
			case 0:
				...
				break;
			case 8:
				data = new Uint8Array(inflate_1.inflate(compressedData, {
					raw: true
				}));
				break;
			default:
	...
	var ZipLoader = function() {
		...
		ZipLoader.prototype.load = function load(compressSize) {
			var _this = this;
			...
			xhr.onload = function(e) {
				...
				_this.files = parseZip(xhr.response,compressSize);
				_this.dispatch({
					type: 'load',
					elapsedTime: Date.now() - startTime
				});
